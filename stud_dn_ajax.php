...
$branch = $dwny['branch'];
$sem = $dwny['sem'];
if ($_POST['idr'] == 13) { //to update the edits done by advisor
  $accno = $_POST['accno'];
  $ifsc = $_POST['ifsc'];
  $branchacc = $_POST['branch'];
  $studidacc = $_POST['studid'];
  if (mysqli_query(
    $conn,
    "UPDATE  registration_details set accno='$accno', ifsc='$ifsc' ,bankbranch='$branchacc' WHERE studid='$studidacc'"
  ))
    echo 1;
  else
    echo 0;
...
if ($_POST['idr'] == 4) { //used to display the list of students
  ...
  $sqQuery = "select * from registration_details where branch='$branch' and batch='$batch' and semester='$sem'and studid in (select studid from to_hod) order by rollno;";
  // echo "1";
...
if ($_POST['idr'] == 3) { //used to return json data for filling
  ...
  $rtarray['4'] = $tdata['category'];
  $rtarray['5'] = $tdata['ifsc'];
  $rtarray['6'] = $tdata['accno'];
  $rtarray['7'] = $tdata['bankbranch'];
